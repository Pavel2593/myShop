...
export const deleteBrands = async (listId) => {
    const response = await $authHost.delete('api/brands/delete', { data: { listId } })
    return response
}

export const updateBrand = async (id, name) => {
    const response = await $authHost.patch('api/brands/update', { id, name })
    return response
}

export const addBrand = async (name) => {
    const response = await $authHost.post('api/brands/add', { name })
    return response
}
